...
void PressureSensorCalibration::setup() {
  ...
  if (std::isnan(this->state) || this->state < this->traits.get_min_value() ||
    ...
    ESP_LOGW(TAG, "  Initial value is invalid, setting to default: %.2f", PRESSURE_SENSOR_CALIBRATION_DEFAULT_VALUE);
    this->publish_state(PRESSURE_SENSOR_CALIBRATION_DEFAULT_VALUE);
  }
